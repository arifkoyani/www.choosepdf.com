...
type AppState = "select" | "uploading" | "enterpassword" | "removing" | "ready";
...
const RemovePasswordFromPdf = () => {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (data.error === false) {
        ...
        // Automatically try to remove password
  ...
  const removePassword = async (fileUrl: string, providedPassword?: string) => {
    setState("removing");
    setErrorMessage("");

    try {
      const response = await fetch("/api/removepasswordfrompdf", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          url: fileUrl,
          password: providedPassword || password || undefined,
          name: `unprotected-${uploadedFiles[0]?.name || Date.now()}.pdf`,
        }),
      });
      ...
      if (!response.ok || data.error === true) {
        ...
        if (data.requiresPassword) {
          ...
          setErrorMessage("This PDF is password protected. Please enter the password.");
          return;
        }

        const errorMsg = data.message || "Remove password failed. Please try again.";
      ...
      if (data.error === false && data.url) {
        setUnprotectedResult({
          url: data.url,
          name: `unprotected-${uploadedFiles[0]?.name || "document.pdf"}`,
        });
        setState("ready");
      } else {
        setErrorMessage("Remove password failed. Please try again.");
        setState("enterpassword");
      }
    } catch (error) {
      console.error("Remove password error:", error);
      const errorMsg = error instanceof Error ? error.message : "Remove password failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("enterpassword");
    }
  };
  ...
  const handleRemovePassword = () => {
    ...
    removePassword(uploadedFiles[0].url, password);
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Remove Password from PDF</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Unlock password-protected PDF files. Remove password protection to access your documents freely.
        </p>
      </div>
      ...
      <Card className="w-full max-w-6xl p-6 sm:p-8 shadow-none border-none bg-transparent">
        <div className="space-y-6">
          ...
          {/* Password Input Section - After Upload if password is required */}
          {uploadedFiles.length > 0 && !unprotectedResult && state === "enterpassword" && (
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg text-left space-y-3">
                <h4 className="font-medium text-sm text-gray-700">Enter PDF Password</h4>
                <p className="text-sm text-gray-600">This PDF is password protected. Please enter the password to unlock it.</p>
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-left">
                <p className="text-blue-700 text-sm">
                  <strong>Note:</strong> If the PDF is not password protected, the password field will be skipped automatically.
                </p>
              </div>
            </div>
          )}
          ...
          {state === "ready" && unprotectedResult && (
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    <FileText className="w-5 h-5 text-red-500" />
                    <div className="text-left flex-1 min-w-0">
                      <p className="font-medium text-sm text-gray-900 truncate">{unprotectedResult.name}</p>
                      <p className="text-xs text-gray-600">Unprotected PDF</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 ml-3">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.open(unprotectedResult.url, "_blank")}
                      title="Open in new tab"
                      className="border-gray-300 hover:bg-gray-100 cursor-pointer"
                    >
                      <MoveUpRight className="w-4 h-4 text-gray-700" />
                    ...
                    <Button
                      onClick={() => downloadFile(unprotectedResult.url, unprotectedResult.name)}
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <Button
                    ...
                    disabled={sendingEmail || !unprotectedResult || !toEmail}
              ...
              <Button onClick={resetConverter} variant="outline" className="mt-2 w-full border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-6 cursor-pointer">
                Remove Password from Another PDF
              </Button>
            </div>
          )}
        </div>
