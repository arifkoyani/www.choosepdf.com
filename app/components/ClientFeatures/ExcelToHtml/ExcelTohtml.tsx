...
const ExcelToHtml = () => {
  ...
  const convertExcelToHtml = async (fileUrl: string, fileName: string) => {
    setState("converting");
    setErrorMessage("");

    try {
      const response = await fetch("/api/exceltohtml", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          url: fileUrl,
          name: fileName.replace(/\.(xls|xlsx|xlsm)$/i, ".html") || "converted-file.html",
        }),
      });
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "Excel to HTML conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "Excel to HTML conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert Excel to HTML</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Upload your Excel file (XLS, XLSX, or XLSM) and convert it to HTML format. Perfect for displaying spreadsheet data on web pages.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your Excel file to convert it to HTML format.
                </p>
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting Excel to HTML...</p>
          ...
          {state === "ready-to-convert" && uploadedFile && (
            <div className="space-y-4">
              ...
              {/* Convert Button */}
              <Button
                onClick={() => convertExcelToHtml(uploadedFile.url, uploadedFile.name)}
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-left">
                <p className="text-blue-700 text-sm">
                  <strong>Note:</strong> Click the button above to convert your Excel file to HTML format. The conversion will process your data and generate an HTML file.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted Excel to HTML!
                </p>
              </div>
              ...
              <div className="pt-2">
                <h3 className="text-sm mb-3 text-gray-900">Share and Receive HTML via Email</h3>
