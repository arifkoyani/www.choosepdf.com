...
const TxtToPdf = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (isValidFileType(file)) {
        ...
        alert("Please select a valid TXT file (.txt)");
      }
    }
  ...
  const convertTxtToPdf = async () => {
    if (!uploadedFile) {
      setErrorMessage("Please upload a TXT file first");
      return;
    }
    ...
    try {
      const response = await fetch("/api/txttopdf", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "TXT to PDF conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "TXT to PDF conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert TXT to PDF</h1>
        <p className="text-sm text-gray-500 text-center max-w-2xl px-4">
          Convert your TXT files to high-quality PDF documents.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your TXT file to convert it to PDF format.
                </p>
                <p className="text-sm text-gray-600 text-center">Supported: TXT</p>
              </span>
            </div>
          )}
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting TXT to PDF...</p>
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted TXT to PDF! {convertResult.urls.length} file(s) converted.
                </p>
              </div>
