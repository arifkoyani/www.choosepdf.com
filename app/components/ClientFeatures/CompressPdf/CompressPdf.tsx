...
type AppState = "select" | "uploading" | "compressing" | "ready";
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (data.error === false) {
        ...
        // Start compression automatically after upload
  ...
  const compressPdf = async (fileUrl: string) => {
    ...
    try {
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "Compression failed. Please try again.";
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Compress PDF</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Automatically reduces PDF size using smart compression without losing quality.
        </p>
      </div>
      ...
      <Card className="w-full max-w-6xl p-6 sm:p-8 shadow-none border-none bg-transparent">
        <div className="space-y-6">
          ...
          {/* Uploaded File Info - Show while compressing */}
          {uploadedFiles.length > 0 && state === "compressing" && (
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    <FileText className="w-5 h-5 text-red-500" />
                    <div className="text-left flex-1 min-w-0">
                      <p className="font-medium text-sm text-gray-900 truncate">{compressResult.name}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 ml-3">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.open(compressResult.url, "_blank")}
                      title="Open in new tab"
                      className="border-gray-300 hover:bg-gray-100 cursor-pointer"
                    >
                      <MoveUpRight className="w-4 h-4 text-gray-700" />
                    ...
                    <Button
                      onClick={() => downloadFile(compressResult.url, compressResult.name)}
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <Button
                    ...
                    disabled={sendingEmail || !compressResult || !toEmail}
