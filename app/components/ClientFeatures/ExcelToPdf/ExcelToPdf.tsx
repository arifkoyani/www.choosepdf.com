...
const ExcelToPdf = () => {
  ...
  const convertExcelToPdf = async (fileUrl: string, fileName: string) => {
    setState("converting");
    setErrorMessage("");
    ...
    try {
      const response = await fetch("/api/exceltopdf", {
        ...
        body: JSON.stringify({
          url: fileUrl,
          name: fileName.replace(/\.(xls|xlsx|xlsm)$/i, ".pdf") || "converted-file.pdf",
        }),
      });
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "Excel to PDF conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "Excel to PDF conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert Excel to PDF</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Upload your Excel file (XLS, XLSX, or XLSM) and convert it to PDF format. Perfect for sharing and archiving spreadsheet data.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your Excel file to convert it to PDF format.
                </p>
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting Excel to PDF...</p>
          ...
          {state === "ready-to-convert" && uploadedFile && (
            <div className="space-y-4">
              ...
              {/* Convert Button */}
              <Button
                onClick={() => convertExcelToPdf(uploadedFile.url, uploadedFile.name)}
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-left">
                <p className="text-blue-700 text-sm">
                  <strong>Note:</strong> Click the button above to convert your Excel file to PDF format. The conversion will process your data and generate a PDF file.
                </p>
              </div>
            </div>
          )}
