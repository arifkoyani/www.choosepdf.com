...
type AppState = "select" | "uploading" | "configuring" | "deleting" | "ready";
...
interface DeleteTextResult {
  url: string;
  name: string;
}

const SearchTextDeleteInPdf = () => {
  const [state, setState] = useState<AppState>("select");
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([]);
  const [deleteTextResults, setDeleteTextResults] = useState<DeleteTextResult[]>([]);
  ...
  const deleteText = async () => {
    ...
    if (!searchString.trim()) {
      setErrorMessage("Please enter a search string to delete");
      return;
    }
    ...
    try {
      const response = await fetch("/api/searchtextdeleteInpdf", {
        method: "POST",
        headers: {
      ...
      const errorMsg = error instanceof Error ? error.message : "Delete text failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("configuring");
    }
  };
  ...
  const handleSendEmail = async () => {
    if (!toEmail || deleteTextResults.length === 0) {
    ...
    try {
      const response = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          toEmail,
          fileUrl: deleteTextResults[0].url,
        }),
      });
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Search Text Delete in PDF</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Remove specific text from your PDF documents. Delete words or sentences from PDF files.
        </p>
      </div>
      ...
      <Card className="w-full max-w-6xl p-6 sm:p-8 shadow-none border-none bg-transparent">
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Delete specific text from your PDF document.
                </p>
          ...
          {uploadedFiles.length > 0 && !deleteTextResults.length && state !== "deleting" && (
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg text-left space-y-3">
                ...
                <div className="space-y-3">
                  <input
                    type="text"
                    placeholder="e.g., Invoice, Confidential, etc."
                    value={searchString}
                    onChange={(e) => {
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <Button
                    ...
                    disabled={sendingEmail || !deleteTextResults[0] || !toEmail}
...
export default SearchTextDeleteInPdf;
