...
const CsvToHtml = () => {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (data.error === false) {
        ...
        convertCsvToHtml(newFile.url, file.name);
      } else {
        setState("select");
  ...
  const convertCsvToHtml = async (fileUrl: string, fileName: string) => {
    setState("converting");
    setErrorMessage("");

    try {
      const response = await fetch("/api/csvtohtml", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          url: fileUrl,
          name: fileName.replace(/\.csv$/i, ".html") || "converted-file.html",
        }),
      });
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "CSV to HTML conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "CSV to HTML conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("select");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert CSV to HTML</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Upload your CSV file and convert it to HTML format. Perfect for displaying spreadsheet data on web pages.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your CSV file to convert it to HTML format.
                </p>
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting CSV to HTML...</p>
              <p className="text-sm text-gray-600">This may take a few moments for file processing...</p>
            </div>
          )}

          {/* Uploaded File Info - Show while converting or ready */}
          {uploadedFile && state !== "select" && state !== "uploading" && (
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted CSV to HTML!
                </p>
              </div>
