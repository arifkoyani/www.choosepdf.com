...
const PdfToJsonByAi = () => {
  ...
  const convertPdfToJsonByAi = async () => {
    if (!uploadedFile) {
    ...
    try {
      const response = await fetch("/api/pdftojsonbyai", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "PDF to JSON by AI conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "PDF to JSON by AI conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  const sendEmail = async () => {
    ...
    if (!convertResult) {
      toast.error("No converted file to send");
      return;
    }
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert PDF to JSON by AI</h1>
        <p className="text-sm text-gray-500 text-center max-w-2xl px-4">
          Convert your PDF files to high-quality JSON format using AI-powered extraction.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your PDF file to convert it to JSON format using AI.
                </p>
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting PDF to JSON by AI...</p>
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted PDF to JSON by AI!
                </p>
              </div>

              {/* Converted File Info */}
              <div className="bg-white p-6 rounded-lg shadow-md space-y-4">
                <h2 className="text-xl font-semibold mb-4 text-gray-900">Converted JSON File</h2>
