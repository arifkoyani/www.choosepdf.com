...
import { X, Download, FileText, MoveUpRight, Loader2, MonitorUp, Merge } from "lucide-react";
...
const MergePdfs = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      if (isValidFileType(file)) {
        ...
        alert(`Please select a valid PDF file. Invalid file: ${file.name}`);
      }
    }
  ...
  const mergePdfs = async () => {
    if (uploadedFiles.length < 2) {
      alert("Please upload at least 2 PDF files to merge. contact choosepdf support team");
      return;
    }
    ...
    try {
      ...
      if (data.error === false && data.url) {
        ...
        console.error("PDF merge failed: contact choosepdf support team");
        alert("contact choosepdf support team");
        setState("select");
      }
    } catch (error) {
  ...
  const handleSendEmail = async () => {
    if (!toEmail || !mergedFileUrl) {
      toast.error("Recipient email and file URL are required");
      return;
    }
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Merge PDFs Into Single PDF</h1>
        <p className="text-sm text-gray-500 text-center max-w-2xl px-4">
          Combine multiple PDF files into one merged PDF document.
        </p>
      </div>
        ...
        <div className="space-y-6">
              ...
              {uploadedFiles.length === 0 && (
                <span className="space-y-2">
                  <p className="text-sm text-gray-600 text-center">
                    Merge PDF from two or more PDF files into a new single PDF.
                  </p>
            ...
            <div className="space-y-4">
              ...
              {uploadedFiles.length >= 2 && (
                ...
                >
                  ...
                  Merge {uploadedFiles.length} PDF{uploadedFiles.length > 1 ? "s" : ""} Now
                </Button>
              )}
            </div>
          )}
          ...
          {state === "merging" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Merging PDFs...</p>
          ...
          {state === "ready" && mergedFileUrl && (
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">Successfully merged {uploadedFiles.length} PDF{uploadedFiles.length > 1 ? "s" : ""} into one PDF!</p>
              </div>
