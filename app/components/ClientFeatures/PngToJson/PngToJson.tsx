...
const PngToJson = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (isValidFileType(file)) {
        ...
        alert("Please select a valid PNG file (.png)");
      }
    }
  ...
  const convertPngToJson = async () => {
    if (!uploadedFile) {
      setErrorMessage("Please upload a PNG file first");
      return;
    }
    ...
    try {
      const response = await fetch("/api/pngtojson", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "PNG to JSON conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "PNG to JSON conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert PNG to JSON</h1>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your PNG image file to convert it to JSON format.
                </p>
                <p className="text-sm text-gray-600 text-center">Supported: PNG</p>
              </span>
            </div>
          )}
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting PNG to JSON...</p>
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted PNG to JSON!
                </p>
              </div>
