...
const CsvToJson = () => {
  ...
  const convertCsvToJson = async (fileUrl: string, fileName: string) => {
    setState("converting");
    setErrorMessage("");
    ...
    try {
      const response = await fetch("/api/csvtojson", {
        ...
        body: JSON.stringify({
          url: fileUrl,
          name: fileName.replace(/\.csv$/i, ".json") || "converted-file.json",
        }),
      });
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "CSV to JSON conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "CSV to JSON conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("select");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert CSV to JSON</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Upload your CSV file and convert it to JSON format. Perfect for data processing and API integration.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your CSV file to convert it to JSON format.
                </p>
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting CSV to JSON...</p>
          ...
          {state === "ready-to-convert" && uploadedFile && (
            <div className="space-y-4">
              ...
              {/* Convert Button */}
              <Button
                onClick={() => convertCsvToJson(uploadedFile.url, uploadedFile.name)}
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-left">
                <p className="text-blue-700 text-sm">
                  <strong>Note:</strong> Click the button above to convert your CSV file to JSON format. The conversion will process your data and generate a JSON file.
                </p>
              </div>
            </div>
          )}
