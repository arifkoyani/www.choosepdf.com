...
const EmailToPdf = () => {
  ...
  const convertEmailToPdf = async () => {
    ...
    try {
      const response = await fetch("/api/emailtopdf", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          name: uploadedFile.name.replace(/\.(eml|msg)$/i, ".pdf") || "email-with-attachments.pdf",
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "Email to PDF conversion failed. Please try again.";
      ...
      if (data.error === false && data.url) {
        ...
        // Handle multiple URLs (though Email to PDF typically returns one)
        setConvertResult({
          url: data.urls[0],
          name: uploadedFile.name.replace(/\.(eml|msg)$/i, ".pdf") || "email-with-attachments.pdf",
        });
      ...
      const errorMsg = error instanceof Error ? error.message : "Email to PDF conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Email to PDF</h1>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Convert email files (.msg or .eml) to PDF. Extract attachments (if any) from input email and embeds into PDF as PDF attachments.
                </p>
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting Email to PDF...</p>
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted Email to PDF!
                </p>
              </div>
