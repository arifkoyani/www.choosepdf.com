...
const PdfToWebp = () => {
  ...
  const convertPdfToWebp = async () => {
    ...
    try {
      const response = await fetch("/api/pdftowebp", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "PDF to WebP conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "PDF to WebP conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert PDF to WebP</h1>
        <p className="text-sm text-gray-500 text-center max-w-2xl px-4">
          Convert your PDF files to high-quality WebP images.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your PDF file to convert it to WebP image format.
                </p>
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting PDF to WebP...</p>
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted PDF to WebP! {convertResult.urls.length} page(s) converted.
                </p>
              </div>

              {/* Converted Files */}
              <div className="bg-white p-6 rounded-lg shadow-md space-y-4">
                <h2 className="text-xl font-semibold mb-4 text-gray-900">Converted WebP Images ({convertResult.urls.length})</h2>
                    ...
                    <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm text-gray-900">Page {index + 1}.webp</p>
                      ...
                      <div className="flex items-center space-x-2 ml-3">
                        ...
                        <Button
                          onClick={() => downloadFile(url, `page-${index + 1}.webp`, index)}
