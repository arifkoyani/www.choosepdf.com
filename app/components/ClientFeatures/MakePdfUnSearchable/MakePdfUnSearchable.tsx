...
interface UnSearchableResult {
  url: string;
  name: string;
}

const MakePdfUnSearchable = () => {
  const [state, setState] = useState<AppState>("select");
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([]);
  const [unSearchableResult, setUnSearchableResult] = useState<UnSearchableResult | null>(null);
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          toEmail,
          fileUrl: unSearchableResult.url,
        }),
      });
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Make PDF Unsearchable</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Convert PDF files into a "text unsearchable" version by converting your PDF into a "scanned" PDF file which is effectively a flat image.
        </p>
      </div>
      ...
      <Card className="w-full max-w-6xl p-6 sm:p-8 shadow-none border-none bg-transparent">
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Convert PDF files into unsearchable documents by converting them to scanned image-based PDFs.
                </p>
          ...
          {/* Uploaded File Info - Show while processing */}
          {uploadedFiles.length > 0 && state === "processing" && (
          ...
          {state === "processing" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Making PDF unsearchable...</p>
              <p className="text-sm text-gray-600">Converting PDF to image-based format...</p>
            </div>
          )}

          {/* Ready State - Download Result */}
          {state === "ready" && unSearchableResult && (
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully created unsearchable PDF!
                </p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    <FileText className="w-5 h-5 text-red-500" />
                    <div className="text-left flex-1 min-w-0">
                      <p className="font-medium text-sm text-gray-900 truncate">{unSearchableResult.name}</p>
                      <p className="text-xs text-gray-600">Image-based unsearchable PDF</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 ml-3">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.open(unSearchableResult.url, "_blank")}
                      title="Open in new tab"
                      className="border-gray-300 hover:bg-gray-100 cursor-pointer"
                    >
                      <MoveUpRight className="w-4 h-4 text-gray-700" />
                    ...
                    <Button
                      onClick={() => downloadFile(unSearchableResult.url, unSearchableResult.name)}
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <Button
                    ...
                    disabled={sendingEmail || !unSearchableResult || !toEmail}
...
export default MakePdfUnSearchable;
