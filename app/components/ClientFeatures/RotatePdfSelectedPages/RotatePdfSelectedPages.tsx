...
type AppState = "select" | "uploading" | "tabs" | "rotating" | "ready";
...
const RotatePdfSelectedPages = () => {
  ...
  const rotatePdf = async () => {
    ...
    if (selectedTab === "range") {
      ...
      const startNum = Number.parseInt(rangeStart.trim(), 10);
      const endNum = Number.parseInt(rangeEnd.trim(), 10);
      ...
      pagesValue = `${rangeStart.trim()}-${rangeEnd.trim()}`;
    } else if (selectedTab === "single") {
      if (!singlePage.trim()) {
        setErrorMessage("Please specify page number to rotate");
        return;
      }
      pagesValue = singlePage.trim();
    }

    setState("rotating");

    try {
      const response = await fetch("/api/rotatepdfselectedpages", {
        method: "POST",
        headers: {
      ...
      const errorMsg = error instanceof Error ? error.message : "Rotation failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("tabs");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Rotate Selected Pages</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Rotate specific pages in your PDF by selecting page numbers and rotation angle.
        </p>
      </div>
      ...
      <Card className="w-full max-w-6xl p-6 sm:p-8 shadow-none border-none bg-transparent">
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Rotate specific pages in your PDF document.
                </p>
          ...
          {uploadedFiles.length > 0 && !rotateResult && state !== "rotating" && (
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                  ...
                  <Tabs value={selectedTab} onValueChange={handleTabChange} className="w-full">
                    ...
                    <TabsContent value="single" className="mt-4">
                      <div className="bg-gray-50 p-4 rounded-lg text-left space-y-3">
                        ...
                        <p className="text-sm text-gray-600">Rotate only this single page from the PDF.</p>
                        <div className="space-y-2">
                          <input
                            type="text"
                            value={singlePage}
                            onChange={(e) => {
                              setSinglePage(e.target.value);
                              setErrorMessage("");
                            }}
                    ...
                    <TabsContent value="range" className="mt-4">
                      <div className="bg-gray-50 p-4 rounded-lg text-left space-y-3">
                        ...
                        <p className="text-sm text-gray-600">Rotate all pages from start to end.</p>
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-blue-600 font-medium">The page numbers are 1-based, meaning the first page is 1 and not 0.</p>
                ...
                {/* Rotation Angle Select */}
                <div className="bg-gray-50 p-4 rounded-lg text-left space-y-3">
                  <h4 className="font-medium text-sm text-gray-700">Rotation Angle</h4>
                  <select
                    value={angle}
                    onChange={(e) => setAngle(Number(e.target.value))}
                    className="w-full px-3 py-2 text-sm border rounded focus:outline-none focus:ring-2 focus:ring-[#ff911d] focus:border-transparent text-gray-900 bg-white"
                  >
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-left">
                <p className="text-blue-700 text-sm">
                  <strong>Note:</strong> Enter the page numbers you want to rotate and select the rotation angle. The page numbers are 1-based (first page is 1).
                </p>
              </div>
            </div>
          )}
          ...
          {state === "rotating" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Rotating PDF pages...</p>
...
export default RotatePdfSelectedPages;
