...
const RtfToPdf = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (isValidFileType(file)) {
        ...
        alert("Please select a valid RTF file (.rtf)");
      }
    }
  ...
  const convertRtfToPdf = async () => {
    if (!uploadedFile) {
      setErrorMessage("Please upload a RTF file first");
      return;
    }
    ...
    try {
      const response = await fetch("/api/rtftopdf", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || data.error === true) {
        const errorMsg = data.message || "RTF to PDF conversion failed. Please try again.";
      ...
      const errorMsg = error instanceof Error ? error.message : "RTF to PDF conversion failed. Please try again.";
      setErrorMessage(errorMsg);
      setState("ready-to-convert");
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Convert RTF to PDF</h1>
        <p className="text-sm text-gray-500 text-center max-w-2xl px-4">
          Convert your RTF files to high-quality PDF documents.
        </p>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Upload your RTF file to convert it to PDF format.
                </p>
                <p className="text-sm text-gray-600 text-center">Supported: RTF</p>
              </span>
            </div>
          )}
          ...
          {state === "converting" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Converting RTF to PDF...</p>
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully converted RTF to PDF! {convertResult.urls.length} file(s) converted.
                </p>
              </div>
                    ...
                    <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm text-gray-900">converted-document-{index + 1}.pdf</p>
                      ...
                      <div className="flex items-center space-x-2 ml-3">
                        ...
                        <Button
                          onClick={() => downloadFile(url, `converted-document-${index + 1}.pdf`, index)}
