...
const MakePdfSearchable = () => {
  ...
  const processSearchablePdf = async (fileUrl: string, fileName: string) => {
    ...
    try {
      const response = await fetch("/api/makepdfsearchable", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#f4f4f5] flex flex-col items-center justify-start py-8">
      <div className="pb-8 flex flex-col justify-center items-center space-y-3">
        <h1 className="text-gray-600 text-xl font-medium">Make PDF Searchable</h1>
        <p className="text-sm text-gray-600 text-center max-w-2xl">
          Convert scanned PDF documents into text-searchable PDFs using OCR. Adds an invisible text layer for search, indexing, and more.
        </p>
      </div>
      ...
      <Card className="w-full max-w-6xl p-6 sm:p-8 shadow-none border-none bg-transparent">
        <div className="space-y-6">
            ...
            <div className="space-y-4 bg-transparent rounded-xl p-4 flex flex-col items-center justify-between">
              ...
              <span>
                <p className="text-sm text-gray-600 text-center">
                  Convert scanned PDFs into searchable documents with OCR technology.
                </p>
          ...
          {state === "processing" && (
            <div className="flex flex-col items-center space-y-4 py-12">
              <Spinner />
              <p className="text-gray-700 font-medium">Making PDF searchable with OCR...</p>
              <p className="text-sm text-gray-600">Running OCR and adding searchable text layer to your PDF...</p>
            </div>
          )}
            ...
            <div className="space-y-5">
              <div className="p-5 bg-transparent border-2 border-transparent rounded-xl">
                <p className="text-[#8f969c] text-center">
                  Successfully created searchable PDF with OCR!
                </p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    <FileText className="w-5 h-5 text-red-500" />
                    <div className="text-left flex-1 min-w-0">
                      <p className="font-medium text-sm text-gray-900 truncate">{searchableResult.name}</p>
                      <p className="text-xs text-gray-600">OCR-enabled searchable PDF</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 ml-3">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.open(searchableResult.url, "_blank")}
                      title="Open in new tab"
                      className="border-gray-300 hover:bg-gray-100 cursor-pointer"
                    >
                      <MoveUpRight className="w-4 h-4 text-gray-700" />
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <Button
                    ...
                    disabled={sendingEmail || !searchableResult || !toEmail}
...
export default MakePdfSearchable;
