  ...
  return (
    <div className="w-full" {...getRootProps()}>
      <motion.div
        onClick={handleClick}
        whileHover="animate"
        className="p-10 group/file block rounded-lg cursor-pointer w-full relative overflow-hidden bg-[#f4f4f5]"
      >
        <input
          ref={fileInputRef}
          ...
          <div className="relative w-full mt-10 max-w-xl mx-auto ">
            ...
            {!files.length && (
              ...
              >
                {isDragActive ? (
                  ...
                  <IconUpload className="h-4 w-4 text-neutral-600 dark:text-[#ff911d]" />
                )}
              </motion.div>
            )}
            ...
            {!files.length && (
              <motion.div
                ...
                className="absolute opacity-0 border border-dashed border-[#ff911d] inset-0 z-30 bg-transparent flex items-center justify-center h-32 mt-4 w-full max-w-[8rem] mx-auto rounded-md"
              ></motion.div>
            )}
          </div>
        </div>
