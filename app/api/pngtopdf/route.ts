...
const PNG_TO_PDF_URL = process.env.CHOOSE_PDF_PNG_TO_PDF_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_PNG_TO_PDF_URL || "https://api.pdf.co/v1/pdf/convert/from/image";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!PNG_TO_PDF_URL) {
      return NextResponse.json(
        { error: true, message: 'PNG to PDF conversion service URL is not configured' },
        { status: 500 }
      );
    }

    // Prepare payload for PNG to PDF conversion API
    const payload = {
      url: url,
      pages: pages || "",
      password: password || "",
    ...
    // Call external PNG to PDF conversion API
    const response = await fetch(PNG_TO_PDF_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'PNG to PDF conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || data?.body?.error || 'PNG to PDF conversion failed';
      console.error('PNG to PDF API error:', {
        status: response.status,
        error: data?.error,
    ...
    if (data.error === false) {
      ...
      try {
        ...
        // URL format: https://[project].supabase.co/storage/v1/object/public/server/uploads/file.png
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'PNG to PDF conversion failed' },
      { status: 400 }
    );
  } catch (error) {
