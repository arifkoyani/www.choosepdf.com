import { NextRequest, NextResponse } from 'next/server';
import { supabase } from '@/lib/supabaseClient';

const API_KEY = process.env.CHOOSE_PDF_API_KEY || process.env.NEXT_PUBLIC_CHOOSE_PDF_API_KEY || "";
const REMOVE_PASSWORD_URL = process.env.CHOOSE_PDF_REMOVE_PASSWORD_FROM_PDF_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_REMOVE_PASSWORD_FROM_PDF_URL || "";

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { url, password, name } = body;

    if (!url) {
      return NextResponse.json(
        { error: true, message: 'File URL is required' },
        { status: 400 }
      );
    }

    // Prepare payload for remove password API
    const payload: any = {
      url: url,
      async: false,
      name: name || `unprotected-${Date.now()}.pdf`
    };

    // Add password only if provided
    if (password) {
      payload.password = password;
    }

    // Call external PDF remove password API
    const response = await fetch(REMOVE_PASSWORD_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(payload),
    });

    // Parse response
    let data;
    try {
      const contentType = response.headers.get('content-type');
      if (contentType && contentType.includes('application/json')) {
        data = await response.json();
      } else {
        const text = await response.text();
        return NextResponse.json(
          { error: true, message: text || 'PDF remove password failed' },
          { status: response.status }
        );
      }
    } catch (parseError) {
      return NextResponse.json(
        { error: true, message: 'Failed to parse response from PDF service' },
        { status: 500 }
      );
    }

    // Handle error response - check if password is required
    if (!response.ok || data.error === true) {
      // Check if password is required (error code 441)
      if (data.errorCode === 441 && !password) {
        return NextResponse.json(
          { 
            error: true, 
            message: 'This PDF is password protected. Please provide the password.',
            requiresPassword: true 
          },
          { status: 400 }
        );
      }

      const errorMessage = data?.message || data?.error || 'PDF remove password failed';
      return NextResponse.json(
        { error: true, message: errorMessage },
        { status: response.ok ? 400 : response.status }
      );
    }

    // Handle success response
    if (data.error === false && data.url) {
      // Delete the original file from Supabase storage after successful conversion
      try {
        // Extract file path from Supabase URL
        // URL format: https://[project].supabase.co/storage/v1/object/public/server/uploads/file.pdf
        const urlObj = new URL(url);
        const pathParts = urlObj.pathname.split('/');
        const serverIndex = pathParts.indexOf('server');
        if (serverIndex !== -1 && serverIndex < pathParts.length - 1) {
          const filePath = pathParts.slice(serverIndex + 1).join('/');
          await supabase.storage.from('server').remove([filePath]);
        }
      } catch (deleteError) {
        // Log error but don't fail the request if deletion fails
        console.error('Error deleting original file from Supabase:', deleteError);
      }

      return NextResponse.json({
        error: false,
        url: data.url
      });
    }

    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'PDF remove password failed' },
      { status: 400 }
    );
  } catch (error) {
    console.error('PDF remove password error:', error);
    return NextResponse.json(
      { error: true, message: 'Internal server error' },
      { status: 500 }
    );
  }
}

