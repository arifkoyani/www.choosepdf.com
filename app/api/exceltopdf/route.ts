...
const EXCEL_TO_PDF_URL = process.env.CHOOSE_PDF_EXCEL_TO_PDF_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_EXCEL_TO_PDF_URL || "";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare payload for Excel to PDF conversion API
    const payload = {
      url: url,
      async: false,
    ...
    // Call external Excel to PDF conversion API
    const response = await fetch(EXCEL_TO_PDF_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'Excel to PDF conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || 'Excel to PDF conversion failed';
      return NextResponse.json(
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'Excel to PDF conversion failed' },
      { status: 400 }
    );
  } catch (error) {
