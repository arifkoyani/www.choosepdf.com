...
const CSV_TO_PDF_URL = process.env.CHOOSE_PDF_CSV_TO_PDF_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_CSV_TO_PDF_URL || "https://api.pdf.co/v1/pdf/convert/from/doc";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare payload for CSV to PDF conversion API
    const payload = {
      url: url,
      pages: "",
      password: "",
    ...
    // Call external CSV to PDF conversion API
    const response = await fetch(CSV_TO_PDF_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'CSV to PDF conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || 'CSV to PDF conversion failed';
      return NextResponse.json(
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'CSV to PDF conversion failed' },
      { status: 400 }
    );
  } catch (error) {
