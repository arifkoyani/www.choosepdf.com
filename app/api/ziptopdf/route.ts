...
const ZIP_TO_PDF_URL = process.env.CHOOSE_PDF_ZIP_TO_PDF_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_ZIP_TO_PDF_URL || "https://api.pdf.co/v1/pdf/convert/from/doc";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ZIP_TO_PDF_URL) {
      return NextResponse.json(
        { error: true, message: 'ZIP to PDF conversion service URL is not configured' },
        { status: 500 }
      );
    }

    // Prepare payload for ZIP to PDF conversion API
    const payload = {
      url: url,
      pages: pages || "",
      password: password || "",
    ...
    // Call external ZIP to PDF conversion API
    const response = await fetch(ZIP_TO_PDF_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'ZIP to PDF conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || data?.body?.error || 'ZIP to PDF conversion failed';
      console.error('ZIP to PDF API error:', {
        status: response.status,
        error: data?.error,
    ...
    if (data.error === false) {
      ...
      try {
        ...
        // URL format: https://[project].supabase.co/storage/v1/object/public/server/uploads/file.zip
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'ZIP to PDF conversion failed' },
      { status: 400 }
    );
  } catch (error) {
    console.error('ZIP to PDF conversion error:', error);
    return NextResponse.json(
