...
const WORD_TO_PDF_URL = process.env.CHOOSE_PDF_WORD_TO_PDF_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_WORD_TO_PDF_URL || "https://api.pdf.co/v1/pdf/convert/from/doc";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare payload for Word to PDF conversion API
    const payload = {
      url: url,
      pages: "",
      password: "",
    ...
    // Call external Word to PDF conversion API
    const response = await fetch(WORD_TO_PDF_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'Word to PDF conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || 'Word to PDF conversion failed';
      return NextResponse.json(
    ...
    if (data.error === false) {
      ...
      try {
        ...
        // URL format: https://[project].supabase.co/storage/v1/object/public/server/uploads/file.docx
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'Word to PDF conversion failed' },
      { status: 400 }
    );
  } catch (error) {
