...
const PDF_TO_HTML_URL = process.env.CHOOSE_PDF_PDF_TO_HTML_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_PDF_TO_HTML_URL || "https://api.pdf.co/v1/pdf/convert/to/html";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!PDF_TO_HTML_URL) {
      return NextResponse.json(
        { error: true, message: 'PDF to HTML conversion service URL is not configured' },
        { status: 500 }
      );
    }

    // Prepare payload for PDF to HTML conversion API
    const payload = {
      url: url,
      pages: pages || "",
      password: password || "",
    ...
    // Call external PDF to HTML conversion API
    const response = await fetch(PDF_TO_HTML_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'PDF to HTML conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || data?.body?.error || 'PDF to HTML conversion failed';
      console.error('PDF to HTML API error:', {
        status: response.status,
        error: data?.error,
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'PDF to HTML conversion failed' },
      { status: 400 }
    );
  } catch (error) {
