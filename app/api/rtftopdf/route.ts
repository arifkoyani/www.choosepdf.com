...
const RTF_TO_PDF_URL = process.env.CHOOSE_PDF_RTF_TO_PDF_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_RTF_TO_PDF_URL || "https://api.pdf.co/v1/pdf/convert/from/doc";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!RTF_TO_PDF_URL) {
      return NextResponse.json(
        { error: true, message: 'RTF to PDF conversion service URL is not configured' },
        { status: 500 }
      );
    }

    // Prepare payload for RTF to PDF conversion API
    const payload = {
      url: url,
      pages: pages || "",
      password: password || "",
    ...
    // Call external RTF to PDF conversion API
    const response = await fetch(RTF_TO_PDF_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'RTF to PDF conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || data?.body?.error || 'RTF to PDF conversion failed';
      console.error('RTF to PDF API error:', {
        status: response.status,
        error: data?.error,
    ...
    if (data.error === false) {
      ...
      try {
        ...
        // URL format: https://[project].supabase.co/storage/v1/object/public/server/uploads/file.rtf
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'RTF to PDF conversion failed' },
      { status: 400 }
    );
  } catch (error) {
