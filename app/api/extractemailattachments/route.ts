...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare payload for email attachment extraction API
    const payload = {
      url: url,
    ...
    // Call external email attachment extraction API
    const response = await fetch(EXTRACT_EMAIL_ATTACHMENTS_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'Email attachment extraction failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || 'Email attachment extraction failed';
      return NextResponse.json(
    ...
    if (data.error === false && data.body) {
      ...
      return NextResponse.json({
        ...
        message: hasAttachments ? undefined : 'No attachments found in this email',
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'Email attachment extraction failed' },
      { status: 400 }
    );
  } catch (error) {
    console.error('Email attachment extraction error:', error);
    return NextResponse.json(
