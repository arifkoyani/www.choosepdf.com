...
const PDF_TO_JPG_URL = process.env.CHOOSE_PDF_PDF_TO_JPG_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_PDF_TO_JPG_URL || "";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare payload for PDF to JPG conversion API
    const payload = {
      url: url,
      pages: pages || "",
      password: password || "",
    ...
    // Call external PDF to JPG conversion API
    const response = await fetch(PDF_TO_JPG_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'PDF to JPG conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || 'PDF to JPG conversion failed';
      return NextResponse.json(
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'PDF to JPG conversion failed' },
      { status: 400 }
    );
  } catch (error) {
