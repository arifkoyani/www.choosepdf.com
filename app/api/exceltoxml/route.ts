...
const EXCEL_TO_XML_URL = process.env.CHOOSE_PDF_EXCEL_TO_XML_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_EXCEL_TO_XML_URL || "";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare payload for Excel to XML conversion API
    const payload = {
      url: url,
      async: false,
    ...
    // Call external Excel to XML conversion API
    const response = await fetch(EXCEL_TO_XML_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'Excel to XML conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || 'Excel to XML conversion failed';
      return NextResponse.json(
    ...
    if (data.error === false && data.url) {
      ...
      try {
        ...
        // URL format: https://[project].supabase.co/storage/v1/object/public/server/uploads/file.xlsx
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'Excel to XML conversion failed' },
      { status: 400 }
    );
  } catch (error) {
    console.error('Excel to XML conversion error:', error);
    return NextResponse.json(
