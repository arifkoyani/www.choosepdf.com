...
const JPG_TO_JSON_URL = process.env.CHOOSE_PDF_JPG_TO_JSON_URL || process.env.NEXT_PUBLIC_CHOOSE_PDF_JPG_TO_JSON_URL || "";

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { url, name, async } = body;
    ...
    // Prepare payload for JPG to JSON conversion API
    const payload = {
      url: url,
    ...
    // Call external JPG to JSON conversion API
    const response = await fetch(JPG_TO_JSON_URL, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        return NextResponse.json(
          { error: true, message: text || 'JPG to JSON conversion failed' },
    ...
    if (!response.ok || data.error === true) {
      const errorMessage = data?.message || data?.error || 'JPG to JSON conversion failed';
      return NextResponse.json(
    ...
    if (data.error === false && data.url) {
      ...
      return NextResponse.json({
        error: false,
        url: data.url,
        name: data.name || name || "result.json",
    ...
    // Fallback error case
    return NextResponse.json(
      { error: true, message: data?.message || 'JPG to JSON conversion failed' },
      { status: 400 }
    );
  } catch (error) {
    console.error('JPG to JSON conversion error:', error);
    return NextResponse.json(
